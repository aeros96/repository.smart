...
SPORTS = {
    ...
    'MMA 1': 'https://fullfightreplays.com'
}

SPORTS2 = {
    'MMA 2': 'https://fullmatchtv.com/category/other-sports/wwe-mma/',
...
def get_links(name:str, url: str, thumbnail: str) -> list:
    ...
    if alt_links:
        for alt_link in alt_links:
            ...
            if link.startswith('//'):
                link = f'https:{link}'
            if 'basketball-video.com' in url:
                soup_ = m.get_soup(link, referer=referer)
...
def categories(url):
    ...
    for a in cats.find_all('a'):
        try:
            ...
            if 'Hard Knocks 2023' in title:
                m.add_dir(title, link, 'replays_links', m.addon_icon, m.addon_fanart, title, isFolder=False)
            else:
